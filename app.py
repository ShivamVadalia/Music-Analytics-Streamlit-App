...
if search_keyword is not None and len(str(search_keyword)) > 0:
    if search_selected == 'Song/Track':
        st.write("Results found for Song/Track")
    ...
    if search_selected == 'Artist':
        st.write("Results found for Artist")
    ...
    if search_selected == 'Album':
        st.write("Results found for Album")
...
if search_selected == 'Album':
    selected_album = st.selectbox("Select your song/track: ",search_results)         
...
if selected_track is not None and len(tracks)>0:
    ...
    if len(tracks_list) > 0:
        for track in tracks_list:
            ...
            if str_temp == selected_track:
                ...
                # st.image(img_album)
                st.subheader('Listen to preview of song')
                if track['preview_url'] is not None:
                    st.audio(track['preview_url'], format="audio/mp3") 
                else:
                    st.write("Oops Song preview is not available")
                break
    if track_id is not None:
        image = songrecommendations.get_album_image(track_id)
        st.image(image)
               
        track_choices = ['Song Features','Similar Song Recommendation']
        ...
        if selected_track_choice == 'Song Features':
        ...
        elif selected_track_choice == 'Similar Song Recommendation':
            ...
            songrecommendations.song_recommendation_viz(recommendation_df)
    else:
    ...
    if len(albums_list) > 0:
            for album in albums_list:
                ...
                # st.write(album['name'] + " - By - " + album['artists'][0]['name'])
                # search_results.append(str_temp)
                # print("Track ID: " + album['id'] + " / Artist ID - " + album['artists'][0]['id'])
    ...
    if album_id is not None and album_uri is not None:
        ...
        df_tracks_min = df_album_tracks.loc[:,['id','name', 'explicit', 'duration_ms', 'preview_url']]
...
if selected_artist is not None and len(artists) > 0:
    ...
    if artist_id is not None and artist_uri is not None:
        ...
        if selected_artist_choice is not None:
            ...
            if selected_artist_choice == 'Top Songs':
                artist_uri = 'spotify:artist:' + artist_id
                top_song_result = sp.artist_top_tracks(artist_uri)
                for track in top_song_result['tracks']:
                    with st.container():
